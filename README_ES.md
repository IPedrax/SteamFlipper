<div align="center">
  ...
  <p>
    ...
    <a href="https://deepwiki.com/OpenSteam001/SteamFlipper">
...
- Soporta la descarga automática de manifiestos a través de las APIs ascendentes (upstream APIs) de `steamflipper` / `steamrun` / `wudrm` (por defecto es steamflipper), o mediante un endpoint personalizado de Lua (ver [Manifest a traves de Lua](#manifest-via-lua)).
...
### Inyección
- Añade inyección opcional de DLL en procesos de juego mediante una o varias entradas `[[inject]]` en `steamflipper.toml`.
...
- Omite las restricciones de Steam Family Sharing para los juegos que se hayan añadido a la biblioteca con `addappid` en Lua. Todas las cuentas de la familia de Steam que participen en el préstamo familiar deben usar SteamFlipper para que esto funcione.

### Compatible con juegos protegidos por Denuvo y SteamStub
- Los juegos protegidos únicamente por SteamStub no requieren la configuración de `AppTicket`. SteamFlipper puede reutilizar el ticket local de ConfigStore de Steam y falsificar el AppId solicitado a través de una vulnerabilidad de desbordamiento por cuatro (off-by-four ticket parsing vulnerability) en SteamDRMP, sin necesidad de inyectarse en el proceso del juego.
...
- Si no hay ningún `setStat` configurado para una aplicación, SteamFlipper consulta `https://stats.steamflipper.com/{appid}` cuando `[stats] enable_api = true` (valor predeterminado).
...
2. Copia los archivos generados `dwmapi.dll`, `xinput1_4.dll` y `SteamFlipper.dll` al directorio raíz de Steam.
...
Cambia el nombre de `steamflipper.example.toml` a `steamflipper.toml` y colócalo en el directorio raíz de Steam (junto a `steam.exe`).
...
[manifest]
# API ascendente para los códigos de solicitud de manifiestos de depósito. Opciones: "steamflipper", "steamrun", "wudrm"
url = "steamflipper"
...
[stats]
# Consulta https://stats.steamflipper.com/{appid} cuando no existe setStat en Lua.
...
# [[inject]]
# path = "SteamFlipperHook.dll"      # un nombre simple se resuelve junto a steam.exe; una ruta absoluta se usa tal cual
...
3.**Caché local** — `<Steam>\steamflipper\pattern\<subdir>\<sha256>.toml`. Se utiliza **únicamente** cuando el servidor remoto no está disponible. La caché se sobrescribe tras cada consulta remota exitosa.
...
Si un paso devuelve un error **HTTP 404**, el bucle de espejos (mirrors) se detiene inmediatamente —todos los espejos sirven el mismo contenido, por lo que un 404 significa que el bot ascendente aún no ha publicado un archivo TOML para esa compilación específica de Steam—. En ese caso, el código recurre a la caché local si existe; de lo contrario, aparecerá una ventana emergente por única vez mostrando el nombre de la DLL no emparejada, su SHA-256, la ruta de caché esperada y la URL de origen. Solo se desactivarán los ganchos (hooks) vinculados a esa DLL; el resto de SteamFlipper seguirá funcionando.
...
Las compilaciones de depuración (Debug) escriben archivos de registro independientes por módulo dentro de `<Steam>/steamflipper/`:
...
El nivel de registro se controla mediante `[log] level` en `steamflipper.toml`.
...
### Archivos de salida
- Debug: `build/Debug/SteamFlipper.dll`, `build/Debug/dwmapi.dll`, `build/Debug/xinput1_4.dll`

- Release: `build/Release/SteamFlipper.dll`, `build/Release/dwmapi.dll`, `build/Release/xinput1_4.dll`
