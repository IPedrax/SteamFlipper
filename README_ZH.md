> [!WARNING]
> **此翻译来自 OpenSteamTool（Windows），内容已过时。**
> SteamFlipper 仅适用于 Linux，下文均不描述本项目。
> 最新文档请见 [README.md](README.md)。
...
- 支持通过 `steamflipper` / `steamrun` / `wudrm` 上游 API 自动下载 manifest（默认为 `steamflipper`），或通过自定义 Lua 端点（参见 [通过 Lua 获取 Manifest](#通过-lua-获取-manifest)）
...
### 注入
- 通过 `steamflipper.toml` 中的一个或多个 `[[inject]]` 条目添加可选的游戏进程 DLL 注入
...
### 家庭共享和远程同乐
- 绕过 Steam 家庭共享限制，适用于通过 Lua 中 `addappid` 添加到库的游戏。参与共享的 Steam 家庭中的所有账户都必须使用 SteamFlipper 才能生效

### 兼容 Denuvo 和 SteamStub 保护的游戏
- 仅 SteamStub 保护的游戏不需要配置 `AppTicket`。SteamFlipper 可以重用 Steam 本地 ConfigStore 令牌，通过 SteamDRMP 令牌解析漏洞伪造请求的 AppId，无需注入游戏进程
- Denuvo 保护的游戏仍需要显式令牌数据。SteamFlipper 通过平台凭据存储保存 `AppTicket` 和 `ETicket`
...
- AppTicket 优先级：显式令牌优先级最高，包括通过 `setAppTicket` 配置的令牌和已缓存的 `AppTicket` 凭据值。若无可用显式 AppTicket，SteamFlipper 回退到伪造的本地 ConfigStore 令牌路径
...
- 如果某个应用未配置 `setStat`，当 `[stats] enable_api = true`（默认）时，SteamFlipper 查询 `https://stats.steamflipper.com/{appid}`
...
2. 将生成的 `dwmapi.dll`、`xinput1_4.dll` 和 `SteamFlipper.dll` 复制到 Steam 根目录
...
将 `steamflipper.example.toml` 重命名为 `steamflipper.toml` 并放在 Steam 根目录（与 `steam.exe` 同级）。
...
[manifest]
# 仓库 manifest 请求码的上游 API。选项："steamflipper"、"steamrun"、"wudrm"
url = "steamflipper"
...
[stats]
# 当没有 Lua setStat 覆盖时查询 https://stats.steamflipper.com/{appid}
...
# [[inject]]
# path = "SteamFlipperHook.dll"      # 裸文件名相对于 steam.exe 解析；绝对路径按原样使用
...
SteamFlipper 不再在 DLL 中内置字节模式签名。相反，每次启动时它计算磁盘上 `steamclient64.dll` 和 `steamui.dll` 的 SHA-256，并从上游跟踪器 [`OpenSteam001/steam-monitor`](https://github.com/OpenSteam001/steam-monitor)（`pattern` 分支）查找匹配的模式文件
...
3. **本地缓存** — `<Steam>\steamflipper\pattern\<subdir>\<sha256>.toml`。仅当远程不可达时使用。每次成功远程获取后覆盖缓存

每次启动都会咨询远程，因此用户自动获取上游重新发布（例如机器人添加新签名或修复现有签名），无需清除任何缓存

如果某步返回 **HTTP 404**，镜像循环立即停止——所有镜像提供相同内容，因此 404 意味着上游机器人尚未为此 Steam 版本发布 TOML。代码然后回退到本地缓存（如果存在）；否则出现一次性弹窗，显示不匹配的 DLL 名称、其 SHA-256、预期缓存路径和上游 URL。仅禁用与该 DLL 相关的钩子——SteamFlipper 的其余部分继续工作
...
调试构建在 `<Steam>/steamflipper/` 下写入每个模块的日志文件：
...
日志级别由 `steamflipper.toml` 中的 `[log] level` 控制
