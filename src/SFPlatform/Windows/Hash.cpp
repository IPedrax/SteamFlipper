...
std::string Sha256OfFile(const std::filesystem::path& path) {
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
        SFP_LOG_WARN("Sha256OfFile: CreateFileW failed for '{}' (error={})",
    ...
    if (!BCRYPT_SUCCESS(status)) {
        SFP_LOG_WARN("Sha256OfFile: BCryptOpenAlgorithmProvider failed (status=0x{:08X})",
                      StatusCode(status));
        CloseHandle(hFile);
        return {};
    }
    ...
    if (!BCRYPT_SUCCESS(status) || hashObjSize == 0) {
        SFP_LOG_WARN("Sha256OfFile: BCryptGetProperty(BCRYPT_OBJECT_LENGTH) failed (status=0x{:08X}, size={})",
    ...
    if (!BCRYPT_SUCCESS(status) || hashSize == 0) {
        SFP_LOG_WARN("Sha256OfFile: BCryptGetProperty(BCRYPT_HASH_LENGTH) failed (status=0x{:08X}, size={})",
    ...
    if (!BCRYPT_SUCCESS(status)) {
        SFP_LOG_WARN("Sha256OfFile: BCryptCreateHash failed (status=0x{:08X})", StatusCode(status));
        cleanup();
        return {};
    }
    ...
    for (;;) {
        ...
        if (!ReadFile(hFile, buf.data(), kChunk, &bytesRead, nullptr)) {
            SFP_LOG_WARN("Sha256OfFile: ReadFile failed for '{}' (error={})",
                          path.string(), GetLastError());
            cleanup();
            return {};
        }
        ...
        if (!BCRYPT_SUCCESS(status)) {
            SFP_LOG_WARN("Sha256OfFile: BCryptHashData failed (status=0x{:08X})", StatusCode(status));
            cleanup();
            return {};
        }
    }
    ...
    if (!BCRYPT_SUCCESS(status)) {
        SFP_LOG_WARN("Sha256OfFile: BCryptFinishHash failed (status=0x{:08X})", StatusCode(status));
        cleanup();
        return {};
    }
    cleanup();
...
std::string Sha256OfBuffer(const void* data, size_t size) {
    ...
    NTSTATUS status = BCryptOpenAlgorithmProvider(&hAlg, BCRYPT_SHA256_ALGORITHM, nullptr, 0);
    if (!BCRYPT_SUCCESS(status)) {
        SFP_LOG_WARN("Sha256OfBuffer: BCryptOpenAlgorithmProvider failed (status=0x{:08X})",
                      StatusCode(status));
        return {};
    }
    ...
    status = BCryptGetProperty(hAlg, BCRYPT_OBJECT_LENGTH, reinterpret_cast<PUCHAR>(&hashObjSize),
                               sizeof(DWORD), &cbData, 0);
    if (!BCRYPT_SUCCESS(status) || hashObjSize == 0) {
        SFP_LOG_WARN("Sha256OfBuffer: BCryptGetProperty(BCRYPT_OBJECT_LENGTH) failed (status=0x{:08X})",
                      StatusCode(status));
        cleanup();
        return {};
    }
    ...
    status = BCryptGetProperty(hAlg, BCRYPT_HASH_LENGTH, reinterpret_cast<PUCHAR>(&hashSize),
                               sizeof(DWORD), &cbData, 0);
    if (!BCRYPT_SUCCESS(status) || hashSize == 0) {
        SFP_LOG_WARN("Sha256OfBuffer: BCryptGetProperty(BCRYPT_HASH_LENGTH) failed (status=0x{:08X})",
                      StatusCode(status));
        cleanup();
        return {};
    }
    ...
    status = BCryptCreateHash(hAlg, &hHash, hashObj.data(), hashObjSize, nullptr, 0, 0);
    if (!BCRYPT_SUCCESS(status)) {
        SFP_LOG_WARN("Sha256OfBuffer: BCryptCreateHash failed (status=0x{:08X})", StatusCode(status));
        cleanup();
        return {};
    }
    ...
    while (remaining != 0) {
        ...
        if (!BCRYPT_SUCCESS(status)) {
            SFP_LOG_WARN("Sha256OfBuffer: BCryptHashData failed (status=0x{:08X})", StatusCode(status));
            cleanup();
            return {};
        }
        cursor    += chunk;
    ...
    status = BCryptFinishHash(hHash, hashBuf.data(), hashSize, 0);
    if (!BCRYPT_SUCCESS(status)) {
        SFP_LOG_WARN("Sha256OfBuffer: BCryptFinishHash failed (status=0x{:08X})", StatusCode(status));
        cleanup();
        return {};
    }
    cleanup();
