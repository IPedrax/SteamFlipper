...
LONG CALLBACK VehThunk(PEXCEPTION_POINTERS exceptionInfo) {
    if (!g_handler) {
        SFP_LOG_TRACE("VehThunk: no handler installed");
        return EXCEPTION_CONTINUE_SEARCH;
    }
    if (!exceptionInfo || !exceptionInfo->ExceptionRecord || !exceptionInfo->ContextRecord) {
        SFP_LOG_DEBUG("VehThunk: incomplete exception context (info={:#x}, record={:#x}, context={:#x})",
    ...
    SFP_LOG_TRACE("VehThunk: exception code=0x{:08X} kind={} ip={:#x}",
                   code, ToString(kind), context.InstructionPointer());

    const bool handled = g_handler(
        kind,
        context);
    SFP_LOG_TRACE("VehThunk: {} at ip={:#x}", handled ? "handled" : "continued", context.InstructionPointer());
...
bool TryReadStackSlot(uintptr_t address, uint64_t& out) {
    ...
    SFP_LOG_TRACE("TryReadStackSlot({:#x}) failed", address);
    return false;
}
...
uint64_t Context::InstructionPointer() const {
    ...
    if (!context) {
        SFP_LOG_TRACE("Context::InstructionPointer: native context is null");
        return 0;
    }
    return context->Rip;
}
...
uint64_t Context::Argument(int index) const {
    PCONTEXT context = AsWindowsContext(nativeContext_);
    if (!context) {
        SFP_LOG_TRACE("Context::Argument({}): native context is null", index);
        return 0;
    }
    ...
    default: {
        if (index <= 0) {
            SFP_LOG_DEBUG("Context::Argument: invalid 1-based index {}", index);
            return 0;
        }
        const uintptr_t slot = context->Rsp + static_cast<uintptr_t>(index) * 8;
        uint64_t value = 0;
        if (!TryReadStackSlot(slot, value)) {
            SFP_LOG_TRACE("Context::Argument({}): stack slot {:#x} is unreadable", index, slot);
            return 0;
        }
        return value;
    }
...
void Context::EnableSingleStep() {
    PCONTEXT context = AsWindowsContext(nativeContext_);
    if (!context) {
        SFP_LOG_TRACE("Context::EnableSingleStep: native context is null");
        return;
    }
    context->EFlags |= 0x100;
    SFP_LOG_TRACE("Context::EnableSingleStep: TF set at ip={:#x}", context->Rip);
}

HandlerHandle AddVectoredHandler(Handler handler) {
    if (!handler) {
        SFP_LOG_DEBUG("AddVectoredHandler: handler is null");
        return nullptr;
    }
    if (g_handle) {
        SFP_LOG_DEBUG("AddVectoredHandler: handler already installed");
        return nullptr;
    }
    ...
    if (!g_handle) {
        SFP_LOG_WARN("AddVectoredExceptionHandler failed (error={})", GetLastError());
        g_handler = nullptr;
    } else {
        SFP_LOG_DEBUG("AddVectoredExceptionHandler installed handle={}", g_handle);
    }
    return g_handle;
}
...
void RemoveVectoredHandler(HandlerHandle handle) {
    if (!handle || handle != g_handle) {
        SFP_LOG_TRACE("RemoveVectoredHandler: ignoring stale handle {}", handle);
        return;
    }

    if (RemoveVectoredExceptionHandler(handle) == 0) {
        SFP_LOG_WARN("RemoveVectoredExceptionHandler failed (error={})", GetLastError());
        return;
    }
    g_handle = nullptr;
    g_handler = nullptr;
    SFP_LOG_DEBUG("RemoveVectoredExceptionHandler removed handle={}", handle);
}
