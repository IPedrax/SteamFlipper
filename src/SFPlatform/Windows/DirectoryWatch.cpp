...
bool Watch::Open(const std::string& directory, uint32_t bufferSize) {
    ...
    if (!impl_->event) {
        SFP_LOG_WARN("DirectoryWatch: CreateEvent failed for '{}' (error={})", directory, GetLastError());
        return false;
    }
    ...
    if (!impl_->dir) {
        SFP_LOG_WARN("DirectoryWatch: CreateFile failed for '{}' (error={})", directory, GetLastError());
        impl_->Close();
        return false;
    }
...
bool Watch::IssueRead() {
    ...
    if (!ReadDirectoryChangesW(
        ...
        if (GetLastError() != ERROR_IO_PENDING) {
            SFP_LOG_WARN("DirectoryWatch: ReadDirectoryChangesW failed for '{}' (error={})",
...
WaitResult WaitAny(std::span<Watch*> watches, uint32_t timeoutMs) {
    ...
    SFP_LOG_WARN("DirectoryWatch: WaitForMultipleObjects failed/result={} error={}", result, GetLastError());
    return {WaitStatus::Failed, 0};
}
