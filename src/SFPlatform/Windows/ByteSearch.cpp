...
std::optional<uint64_t> FindInFileRange(
    ...
    if (!file) {
        SFP_LOG_DEBUG("ByteSearch::FindInFileRange open failed path={} offset=0x{:X} size={} err={}",
    ...
    if (!::GetFileSizeEx(file.get(), &sizeLi) || sizeLi.QuadPart <= 0) {
        SFP_LOG_DEBUG("ByteSearch::FindInFileRange empty/invalid file path={} offset=0x{:X} size={}",
    ...
    if (offset >= fileSize) {
        SFP_LOG_DEBUG("ByteSearch::FindInFileRange offset past EOF path={} offset=0x{:X} file_size={}",
    ...
    if (!events[0] || !events[1]) {
        SFP_LOG_DEBUG("ByteSearch::FindInFileRange event create failed path={} err={}",
    ...
    uint64_t curStart = offset;
    {
        ...
        if (!issueRead(slot, curStart, len)) {
            SFP_LOG_DEBUG("ByteSearch::FindInFileRange read start failed path={} offset=0x{:X} err={}",
    ...
    while (true) {
        ...
        if (!ok) {
            ...
            if (err != ERROR_HANDLE_EOF) {
                SFP_LOG_DEBUG("ByteSearch::FindInFileRange read failed path={} offset=0x{:X} err={}",
        ...
        if (match) {
            ...
            SFP_LOG_DEBUG("ByteSearch::FindInFileRange matched path={} range=[0x{:X},0x{:X}) match=0x{:X} bytes_read={} chunks={} read_ms={:.3f} bmh_ms={:.3f} total_ms={:.3f}",
                           path.string(),
                           offset,
                           rangeEnd,
                           absoluteMatch,
    ...
    SFP_LOG_DEBUG("ByteSearch::FindInFileRange no match path={} range=[0x{:X},0x{:X}) bytes_read={} chunks={} read_ms={:.3f} bmh_ms={:.3f} total_ms={:.3f}",
                   path.string(),
                   offset,
                   rangeEnd,
                   bytesReadTotal,
