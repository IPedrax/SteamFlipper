...
namespace NtAbi = SFPlatform::Windows::NtAbi;
...
Windows::UniqueHandle OpenProcessHandle(uint32_t pid, DWORD access) {
    ...
    if (!process) {
        SFP_LOG_DEBUG("OpenProcess(pid={}, access=0x{:X}) failed (error={})", pid, access, GetLastError());
    }
    return process;
}
...
bool TryReadProcessMemory(HANDLE process, const void* address, void* buffer, size_t size, size_t* bytesRead = nullptr) {
    ...
    if (!ok) {
        SFP_LOG_TRACE("ReadProcessMemory(addr={:#x}, size={}) failed (read={}, error={})",
...
std::optional<uint64_t> GetCreationTime(uint32_t pid) {
    ...
    if (!GetProcessTimes(process.get(), &creation, &exitTime, &kernel, &user)) {
        SFP_LOG_DEBUG("GetProcessTimes(pid={}) failed (error={})", pid, GetLastError());
        return std::nullopt;
    }
...
std::string FormatCreationTime(uint64_t fileTime) {
    ...
    if (!FileTimeToLocalFileTime(&utc, &local)) {
        SFP_LOG_DEBUG("FileTimeToLocalFileTime(filetime={}) failed (error={})", fileTime, GetLastError());
        return std::format("filetime={}", fileTime);
    }

    SYSTEMTIME system{};
    if (!FileTimeToSystemTime(&local, &system)) {
        SFP_LOG_DEBUG("FileTimeToSystemTime(filetime={}) failed (error={})", fileTime, GetLastError());
        return std::format("filetime={}", fileTime);
    }
...
std::optional<std::string> GetImagePath(uint32_t pid) {
    ...
    for (DWORD capacity : {static_cast<DWORD>(MAX_PATH), DWORD{32768}}) {
        ...
        if (GetLastError() != ERROR_INSUFFICIENT_BUFFER) {
            SFP_LOG_DEBUG("QueryFullProcessImageNameW(pid={}) failed (error={})", pid, GetLastError());
            return std::nullopt;
        }
    }
    SFP_LOG_DEBUG("QueryFullProcessImageNameW(pid={}) failed: path exceeds 32768 wchars", pid);
    return std::nullopt;
}
...
std::vector<ModuleInfo> EnumerateModules(uint32_t pid) {
    ...
    if (!snapshot) {
        SFP_LOG_DEBUG("CreateToolhelp32Snapshot(pid={}) failed (error={})", pid, GetLastError());
        return modules;
    }
...
bool IsSystemModulePath(const std::string& path) {
    static const std::string systemDir = [] {
        ...
        if (needed == 0) {
            SFP_LOG_WARN("GetWindowsDirectoryW(size query) failed (error={})", GetLastError());
            return std::string{};
        }
        std::wstring dir(needed, L'\0');
        const UINT written = GetWindowsDirectoryW(dir.data(), needed);
        if (written == 0 || written >= needed) {
            SFP_LOG_WARN("GetWindowsDirectoryW failed (written={}, error={})", written, GetLastError());
            return std::string{};
        }
        dir.resize(written);
...
bool LaunchDetachedHidden(const std::string& commandLine) {
    ...
    for (DWORD extra : { DWORD{CREATE_BREAKAWAY_FROM_JOB}, DWORD{0} }) {
        ...
        SFP_LOG_DEBUG("LaunchDetachedHidden: CreateProcessW failed (extra=0x{:X}, error={})",
                       extra, GetLastError());
    }
    SFP_LOG_WARN("LaunchDetachedHidden: could not launch '{}'", commandLine);
    return false;
}
