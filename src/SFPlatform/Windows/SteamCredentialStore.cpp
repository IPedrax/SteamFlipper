...
LSTATUS ReadBinaryValue(uint32_t appId, PCWSTR valueName, std::vector<uint8_t>& out) {
    ...
    if (status != ERROR_SUCCESS) {
        ...
        if (mapped == Status::NotFound) {
            SFP_LOG_DEBUG("SteamCredentialStore: {} missing for appid={} status={}",
                           ValueNameForLog(valueName), appId, status);
        } else {
            SFP_LOG_WARN("SteamCredentialStore: failed to query {} size for appid={} status={}",
                          ValueNameForLog(valueName), appId, status);
        }
        return status;
    }
    if (bytes == 0) {
        SFP_LOG_DEBUG("SteamCredentialStore: {} empty for appid={}", ValueNameForLog(valueName), appId);
        out.clear();
    ...
    if (status != ERROR_SUCCESS) {
        SFP_LOG_WARN("SteamCredentialStore: failed to read {} for appid={} status={}",
                      ValueNameForLog(valueName), appId, status);
        return status;
    }
    ...
    buffer.resize(bytes);
    out = std::move(buffer);
    SFP_LOG_DEBUG("SteamCredentialStore: read {} for appid={} bytes={}",
...
LSTATUS WriteValue(uint32_t appId, PCWSTR valueName, DWORD type, const BYTE* data, DWORD bytes) {
    ...
    if (status != ERROR_SUCCESS) {
        SFP_LOG_WARN("SteamCredentialStore: failed to create/open app key appid={} status={}", appId, status);
        return status;
    }

    status = RegSetValueExW(key.get(), valueName, 0, type, data, bytes);
    if (status != ERROR_SUCCESS) {
        SFP_LOG_WARN("SteamCredentialStore: failed to write {} for appid={} bytes={} status={}",
                      ValueNameForLog(valueName), appId, bytes, status);
        return status;
    }

    SFP_LOG_INFO("SteamCredentialStore: wrote {} for appid={} bytes={}",
...
Status GetSteamId(uint32_t appId, uint64_t& steamId) {
    ...
    if (status != ERROR_SUCCESS) {
        const auto mapped = FromLStatus(status);
        if (mapped == Status::NotFound) {
            SFP_LOG_DEBUG("SteamCredentialStore: SteamID missing for appid={} status={}", appId, status);
        } else {
            SFP_LOG_WARN("SteamCredentialStore: failed to query SteamID size for appid={} status={}", appId, status);
        }
        return mapped;
    }
    if (bytes < sizeof(wchar_t)) {
        SFP_LOG_WARN("SteamCredentialStore: SteamID value too short for appid={} bytes={}", appId, bytes);
        return Status::NotFound;
    }
    ...
    if (status != ERROR_SUCCESS) {
        SFP_LOG_WARN("SteamCredentialStore: failed to read SteamID for appid={} status={}", appId, status);
    ...
    if (!parsed || *parsed == 0) {
        SFP_LOG_WARN("SteamCredentialStore: SteamID value invalid for appid={}", appId);
        return Status::NotFound;
    }

    steamId = *parsed;
    SFP_LOG_DEBUG("SteamCredentialStore: read SteamID for appid={} steamid={}", appId, steamId);
    return Status::Ok;
}
...
Status GetActiveUser(uint32_t& accountId, std::wstring& universe) {
    ...
    if (status != ERROR_SUCCESS) {
        const auto mapped = FromLStatus(status);
        if (mapped == Status::NotFound) {
            SFP_LOG_DEBUG("SteamCredentialStore: ActiveUser missing status={}", status);
        } else {
            SFP_LOG_WARN("SteamCredentialStore: failed to read ActiveUser status={}", status);
        }
        return mapped;
    }
    if (bytes != sizeof(activeUser)) {
        SFP_LOG_WARN("SteamCredentialStore: ActiveUser has unexpected size bytes={}", bytes);
        return Status::NotFound;
    }
    if (activeUser == 0) {
        SFP_LOG_DEBUG("SteamCredentialStore: ActiveUser is zero");
        return Status::NotFound;
    }
    ...
    if (status != ERROR_SUCCESS) {
        const auto mapped = FromLStatus(status);
        if (mapped == Status::NotFound) {
            SFP_LOG_DEBUG("SteamCredentialStore: ActiveProcess Universe missing status={}", status);
        } else {
            SFP_LOG_WARN("SteamCredentialStore: failed to query ActiveProcess Universe size status={}", status);
        }
        return mapped;
    }
    if (bytes < sizeof(wchar_t)) {
        SFP_LOG_WARN("SteamCredentialStore: ActiveProcess Universe value too short bytes={}", bytes);
        return Status::NotFound;
    }
    ...
    if (status != ERROR_SUCCESS) {
        SFP_LOG_WARN("SteamCredentialStore: failed to read ActiveProcess Universe status={}", status);
        return FromLStatus(status);
    }
    ...
    if (rawUniverse.empty()) {
        SFP_LOG_WARN("SteamCredentialStore: ActiveProcess Universe is empty");
        return Status::NotFound;
    }

    accountId = activeUser;
    universe = std::move(rawUniverse);
    SFP_LOG_DEBUG("SteamCredentialStore: read ActiveUser accountid={} universe={}({} chars)",
