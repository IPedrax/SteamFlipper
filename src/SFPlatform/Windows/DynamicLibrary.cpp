...
std::string WidePathToUtf8(std::wstring_view path, const char* operation) {
    ...
    if (result.empty() && !path.empty()) {
        SFP_LOG_DEBUG("{}: failed to convert path to UTF-8", operation);
    }
    return result;
}
...
std::filesystem::path ModuleFilePath(HMODULE module, const char* operation) {
    for (DWORD capacity = MAX_PATH; capacity <= kMaxWin32PathBuffer; capacity *= 2) {
        ...
        if (len == 0) {
            SFP_LOG_WARN("{}: GetModuleFileNameW failed (error={})", operation, GetLastError());
            return {};
        }
        if (len < capacity) {
    ...
    SFP_LOG_WARN("{}: GetModuleFileNameW path exceeds {} wchars", operation, kMaxWin32PathBuffer);
    return {};
}
...
std::string DirectoryFromQuery(DWORD (*query)(DWORD, wchar_t*), const char* operation) {
    for (DWORD capacity = MAX_PATH; capacity <= kMaxWin32PathBuffer; capacity *= 2) {
        ...
        if (len == 0) {
            SFP_LOG_WARN("{} failed (error={})", operation, GetLastError());
            return {};
        }
        if (len < capacity) {
    ...
    SFP_LOG_WARN("{} path exceeds {} wchars", operation, kMaxWin32PathBuffer);
    return {};
}
...
ModuleHandle Load(const std::filesystem::path& path) {
    ...
    if (!module) {
        SFP_LOG_WARN("LoadLibraryW('{}') failed (error={})", path.string(), GetLastError());
    }
    return module;
}
...
ModuleHandle GetLoaded(std::string_view moduleName) {
    ...
    if (wideName.empty() && !moduleName.empty()) {
        SFP_LOG_DEBUG("GetLoaded('{}'): module name conversion failed", moduleName);
        return nullptr;
    }
...
void* GetSymbol(ModuleHandle module, const char* symbolName) {
    ...
    if (!symbol) {
        SFP_LOG_DEBUG("GetProcAddress('{}') failed (error={})", symbolName ? symbolName : "", GetLastError());
    }
    return symbol;
}
...
void* GetSymbol(ModuleHandle module, uint16_t ordinal) {
    ...
    if (!symbol) {
        SFP_LOG_DEBUG("GetProcAddress(ordinal={}) failed (error={})", ordinal, GetLastError());
    }
    return symbol;
}

void* GetModuleBase(ModuleHandle module) {
    // An HMODULE already is the module's load address.
    return module;
}
