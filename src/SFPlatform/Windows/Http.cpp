...
Result Execute(const wchar_t* method,
    ...
    if (!pu.valid) {
        SFP_LOG_WARN("Invalid URL: {}", url ? url : "");
        return r;
    }

    auto t0 = std::chrono::steady_clock::now();

    HINTERNET hSession = WinHttpOpen(L"SteamFlipper/1.0",
        WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
        WINHTTP_NO_PROXY_NAME,
        WINHTTP_NO_PROXY_BYPASS,
        0);
    if (!hSession) {
        SFP_LOG_WARN("{} - WinHttpOpen failed (error={})", url ? url : "", GetLastError());
        return r;
    }
    ...
    if (!hConnect) {
        SFP_LOG_WARN("{} - WinHttpConnect(host='{}', port={}) failed (error={})",
                      url ? url : "",
    ...
    HINTERNET hRequest = WinHttpOpenRequest(
        ...
        flags);
    if (!hRequest) {
        SFP_LOG_WARN("{} - WinHttpOpenRequest failed (error={})", url ? url : "", GetLastError());
    ...
    if (headers && headers[0]) {
        if (!WinHttpAddRequestHeaders(
            ...
            SFP_LOG_WARN("{} - WinHttpAddRequestHeaders failed (error={})", url ? url : "", GetLastError());
        }
    }
    ...
    if (!WinHttpSendRequest(hRequest, WINHTTP_NO_ADDITIONAL_HEADERS, 0,
        ...
        SFP_LOG_WARN("{} - WinHttpSendRequest failed (error={})", url, GetLastError());
    } else if (!WinHttpReceiveResponse(hRequest, nullptr)) {
        SFP_LOG_WARN("{} - WinHttpReceiveResponse failed (error={})", url, GetLastError());
    } else {
        ...
        if (!WinHttpQueryHeaders(
            ...
            SFP_LOG_WARN("{} - WinHttpQueryHeaders(status) failed (error={})", url ? url : "", GetLastError());
        }

        DWORD avail = 0;
        while (true) {
            if (!WinHttpQueryDataAvailable(hRequest, &avail)) {
                SFP_LOG_WARN("{} - WinHttpQueryDataAvailable failed (error={})", url ? url : "", GetLastError());
                break;
            }
            if (!avail) break;
            ...
            if (!WinHttpReadData(hRequest, r.body.data() + off, avail, &read)) {
                SFP_LOG_WARN("{} - WinHttpReadData(size={}) failed (error={})",
        ...
        if (r.status < 200 || r.status >= 300) {
            SFP_LOG_WARN("{} - unexpected HTTP {}  body={}",
                             url,
                             r.status,
                             r.body.size() > 512 ? r.body.substr(0, 512) + "..." : r.body);
        } else {
            SFP_LOG_TRACE("{} - response body={} ({}bytes)",
