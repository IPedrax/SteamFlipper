...
FileRead ReadFileRange(const std::filesystem::path& path, uint64_t offset, size_t requestedSize) {
    ...
    if (!file) {
        SFP_LOG_DEBUG("PE::ReadFileRange open failed path={} offset=0x{:X} requested={} err={}",
    ...
    if (!::GetFileSizeEx(file.get(), &sizeLi) || sizeLi.QuadPart <= 0) {
        SFP_LOG_DEBUG("PE::ReadFileRange empty/invalid file path={} offset=0x{:X} requested={}",
    ...
    if (offset >= fileSize) {
        SFP_LOG_DEBUG("PE::ReadFileRange offset past EOF path={} offset=0x{:X} file_size={} elapsed_ms={:.3f}",
    ...
    if (!::SetFilePointerEx(file.get(), seekTo, nullptr, FILE_BEGIN)) {
        SFP_LOG_DEBUG("PE::ReadFileRange seek failed path={} offset=0x{:X} err={}",
    ...
    if (!ReadFull(file.get(), bytes.data(), readSize)) {
        SFP_LOG_DEBUG("PE::ReadFileRange read failed path={} offset=0x{:X} requested={} read_size={} err={} elapsed_ms={:.3f}",
                       path.string(), offset, requestedSize, readSize, ::GetLastError(), totalTimer.ElapsedMs());
        return {};
    }
    const double readMs = readTimer.ElapsedMs();

    SFP_LOG_DEBUG("PE::ReadFileRange path={} offset=0x{:X} requested={} read={} file_size={} read_ms={:.3f} total_ms={:.3f}",
...
ByteBuffer ReadFileBytes(const std::filesystem::path& path) {
    ...
    const Windows::UniqueFileHandle file = OpenForSequentialRead(path);
    if (!file) {
        SFP_LOG_DEBUG("PE::ReadFileBytes open failed path={} err={}", path.string(), ::GetLastError());
        return {};
    }

    LARGE_INTEGER sizeLi{};
    if (!::GetFileSizeEx(file.get(), &sizeLi) || sizeLi.QuadPart <= 0) {
        SFP_LOG_DEBUG("PE::ReadFileBytes empty/invalid file path={}", path.string());
        return {};
    }
    ...
    if (!ReadFull(file.get(), bytes.data(), size)) {
        SFP_LOG_DEBUG("PE::ReadFileBytes read failed path={} size={} err={} elapsed_ms={:.3f}",
                       path.string(), static_cast<uint64_t>(size), ::GetLastError(), totalTimer.ElapsedMs());
        return {};
    }
    const double readMs = readTimer.ElapsedMs();

    SFP_LOG_DEBUG("PE::ReadFileBytes path={} size={} read_ms={:.3f} total_ms={:.3f}",
...
Image::Image(const std::filesystem::path& path) : path_(path) {
    ...
    if (headerBytes_.empty()) {
        SFP_LOG_DEBUG("PE::Image invalid empty header path={} elapsed_ms={:.3f}",
    ...
    if (!dos || dos->e_magic != IMAGE_DOS_SIGNATURE || dos->e_lfanew < 0) {
        SFP_LOG_DEBUG("PE::Image invalid DOS header path={} elapsed_ms={:.3f}",
                       path.string(), totalTimer.ElapsedMs());
        return;
    }
    ...
    if (!signature || *signature != IMAGE_NT_SIGNATURE) {
        SFP_LOG_DEBUG("PE::Image invalid NT signature path={} nt_offset=0x{:X} elapsed_ms={:.3f}",
    ...
    if (!fileHeader || fileHeader->NumberOfSections == 0) {
        SFP_LOG_DEBUG("PE::Image invalid file header path={} elapsed_ms={:.3f}",
                       path.string(), totalTimer.ElapsedMs());
        return;
    }
    ...
    if (!magic) {
        SFP_LOG_DEBUG("PE::Image missing optional header path={} elapsed_ms={:.3f}",
                       path.string(), totalTimer.ElapsedMs());
        return;
    }
    ...
    if (*magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC) {
        ...
        SFP_LOG_DEBUG("PE::Image unsupported optional header path={} magic=0x{:X} elapsed_ms={:.3f}",
    ...
    if (sectionsEnd > headerBytes_.size()) {
        SFP_LOG_DEBUG("PE::Image section headers outside header buffer path={} sections={} elapsed_ms={:.3f}",
    ...
    for (WORD i = 0; i < fileHeader->NumberOfSections; ++i) {
        ...
        if (!section) {
            SFP_LOG_DEBUG("PE::Image missing section header path={} index={} elapsed_ms={:.3f}",
    ...
    valid_ = true;
    SFP_LOG_DEBUG("PE::Image parsed path={} file_size={} sections={} entry=0x{:X} elapsed_ms={:.3f}",
