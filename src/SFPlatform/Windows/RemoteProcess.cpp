...
Architecture DetectArchitecture(HANDLE process) {
    ...
    if (!IsWow64Process(process, &wow64)) {
        SFP_LOG_DEBUG("IsWow64Process failed (error={})", GetLastError());
...
std::vector<RemoteModule> EnumerateRemoteModules(uint32_t pid, Architecture architecture) {
    ...
    if (!snapshot) {
        SFP_LOG_WARN("CreateToolhelp32Snapshot(pid={}, flags=0x{:X}) failed (error={})",
...
std::optional<uintptr_t> ResolveRemoteExport(
    ...
    if (!module) {
        SFP_LOG_WARN("ResolveRemoteExport: module '{}' not found", Encoding::WideToUtf8(moduleName));
        return std::nullopt;
    }
    ...
    if (!exported) {
        SFP_LOG_WARN("ResolveRemoteExport: export {}!{} not found",
    ...
    if (dot == std::string::npos || dot == 0 || dot + 1 >= exported->forwarder.size()) {
        SFP_LOG_WARN("ResolveRemoteExport: unsupported forwarder '{}'", exported->forwarder);
        return std::nullopt;
    }
    ...
    if (!forwardSymbol.empty() && forwardSymbol.front() == '#') {
        SFP_LOG_WARN("ResolveRemoteExport: ordinal forwarder '{}' is not supported", exported->forwarder);
        return std::nullopt;
    }
...
LPTHREAD_START_ROUTINE ResolveRemoteLoadLibraryW(uint32_t pid, Architecture architecture) {
    ...
    if (!address) {
        SFP_LOG_WARN("ResolveRemoteLoadLibraryW(pid={}, arch={}) failed", pid, ToString(architecture));
        return nullptr;
    }
...
Architecture GetArchitecture(uint32_t pid) {
    ...
    if (!process) {
        SFP_LOG_DEBUG("OpenProcess(pid={}, access=0x{:X}) failed (error={})",
...
InjectStatus InjectLibrary(uint32_t pid, const std::filesystem::path& libraryPath) {
    ...
    if (!process) {
        SFP_LOG_WARN("OpenProcess(pid={}, access=0x{:X}) failed (error={})", pid, kInjectAccess, GetLastError());
        return InjectStatus::OpenProcessFailed;
    }

    const Architecture architecture = DetectArchitecture(process.get());
    if (architecture == Architecture::Unknown) {
        SFP_LOG_WARN("InjectLibrary(pid={}): target architecture is unknown", pid);
    ...
    if (!remotePath) {
        SFP_LOG_WARN("VirtualAllocEx(size={}) failed (error={})", byteSize, GetLastError());
    ...
    if (!WriteProcessMemory(process.get(), remotePath, nativePath.c_str(), byteSize, &written) ||
        written != byteSize) {
        SFP_LOG_WARN("WriteProcessMemory(size={}) failed (written={}, error={})",
    ...
    if (!thread) {
        SFP_LOG_WARN("CreateRemoteThread failed (error={})", GetLastError());
    ...
    if (wait != WAIT_OBJECT_0) {
        if (wait == WAIT_FAILED) {
            SFP_LOG_WARN("WaitForSingleObject(remote thread) failed (error={})", GetLastError());
        } else if (wait == WAIT_TIMEOUT) {
            SFP_LOG_WARN("WaitForSingleObject(remote thread) timed out after {} ms", kRemoteLoadTimeoutMs);
        } else {
            SFP_LOG_WARN("WaitForSingleObject(remote thread) returned unexpected status {}", wait);
        }
    ...
    if (!GetExitCodeThread(thread.get(), &exitCode)) {
        SFP_LOG_WARN("GetExitCodeThread(remote thread) failed (error={})", GetLastError());
        return InjectStatus::RemoteLoadFailed;
    }
    if (exitCode == 0) {
        SFP_LOG_WARN("Remote LoadLibraryW returned 0 for '{}'", libraryPath.string());
        return InjectStatus::RemoteLoadFailed;
    }
