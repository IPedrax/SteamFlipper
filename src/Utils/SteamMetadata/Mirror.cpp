...
namespace {
    // Delivery mirror chain for the `updates` branch of madoiscool/SteamFlipper.
    ...
    constexpr const char* kBaseTemplates[] = {
        "https://raw.githubusercontent.com/madoiscool/SteamFlipper/updates/{path}",
        "https://cdn.jsdelivr.net/gh/madoiscool/SteamFlipper@updates/{path}",
        "https://git.lua.tools/luatools/SteamFlipper/raw/branch/updates/{path}",
    };
...
std::optional<std::string> Fetch(std::string_view relPath)
{
    for (size_t i = 0; i < std::size(kBaseTemplates); ++i) {
        ...
        SFPlatform::Http::Result http =
            SFPlatform::Http::Execute(L"GET", url.c_str(), nullptr, 0, nullptr,
