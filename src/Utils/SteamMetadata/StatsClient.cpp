...
namespace {
    ...
    bool ParseSteamId(std::string_view body, uint64_t* outSteamId) {
        const auto parsed = SFPlatform::Numbers::ParseUInt64(body);
...
bool FetchStatSteamId(AppId_t appId, uint64_t* outSteamId) {
    ...
    char url[128];
    std::snprintf(url, sizeof(url), "https://stats.steamflipper.com/%u", appId);

    auto r = SFPlatform::Http::Execute(L"GET", url);
