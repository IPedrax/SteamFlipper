...
namespace AppTicket {
    ...
    static uint64_t GetSteamIDFromCredentialStore(AppId_t appId) {
        uint64_t steamId = 0;
        const auto status = SFPlatform::SteamCredentialStore::GetSteamId(appId, steamId);
        if (status != SFPlatform::SteamCredentialStore::Status::Ok) {
            LOG_TRACE("GetSpoofSteamID for AppId {}: SteamID unavailable in credential store ({})",
                      appId, SFPlatform::SteamCredentialStore::ToString(status));
            return 0;
        }
    ...
    std::vector<uint8_t> GetAppOwnershipTicketFromCredentialStore(AppId_t appId) {
        ...
        const auto status = SFPlatform::SteamCredentialStore::GetAppTicket(appId, ticket);
        if (status != SFPlatform::SteamCredentialStore::Status::Ok) {
    ...
    std::vector<uint8_t> GetEncryptedTicketFromCredentialStore(AppId_t appId) {
        ...
        std::vector<uint8_t> ticket;
        const auto status = SFPlatform::SteamCredentialStore::GetETicket(appId, ticket);
        if (status != SFPlatform::SteamCredentialStore::Status::Ok) {
    ...
    bool WriteAppOwnershipTicket(AppId_t appId, const std::vector<uint8_t>& data) {
        ...
        const auto status = SFPlatform::SteamCredentialStore::WriteAppTicket(appId, data);
        if (status != SFPlatform::SteamCredentialStore::Status::Ok) {
    ...
    bool WriteEncryptedTicket(AppId_t appId, const std::vector<uint8_t>& data) {
        // we can't execlude appids here 
        const auto status = SFPlatform::SteamCredentialStore::WriteETicket(appId, data);
        if (status != SFPlatform::SteamCredentialStore::Status::Ok) {
    ...
    bool WriteSteamID(AppId_t appId, uint64_t steamId) {
        const auto status = SFPlatform::SteamCredentialStore::WriteSteamId(appId, steamId);
        if (status != SFPlatform::SteamCredentialStore::Status::Ok) {
