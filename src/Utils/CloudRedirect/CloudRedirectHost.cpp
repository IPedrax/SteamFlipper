...
namespace {
    ...
    // Declared locally so SteamFlipper does not need CloudRedirect's headers.
    ...
    SFPlatform::DynamicLibrary::ModuleHandle g_module = nullptr;
    ...
    // Routes CloudRedirect's notifications into SteamFlipper's log instead of
...
#else
            return std::filesystem::path(steamRoot) / "cloud_redirect.so";
#endif
        }
    ...
    template <typename T>
    bool ResolveSymbol(SFPlatform::DynamicLibrary::ModuleHandle module,
                       const char* name, T& out) {
        out = reinterpret_cast<T>(SFPlatform::DynamicLibrary::GetSymbol(module, name));
        if (!out) {
...
void Initialize(const char* steamInstallPath) {
    ...
    g_module = SFPlatform::DynamicLibrary::Load(libPath);
    if (!g_module) {
        LOG_WARN("CloudRedirect: failed to load {} (err={})",
                 libPath.string(), SFPlatform::DynamicLibrary::GetLastErrorCode());
        return;
    }
