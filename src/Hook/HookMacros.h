...
#include "SFPlatform/include/Detour.h"
...
#define HOOK_BEGIN()                          \
    do {                                       \
        bool _ost_detour_transaction_ok_ = SFPlatform::Detour::BeginTransaction()

#define HOOK_END()                            \
        _ost_detour_transaction_ok_ = SFPlatform::Detour::CommitTransaction() && _ost_detour_transaction_ok_; \
        if (!_ost_detour_transaction_ok_)                                                                      \
            LOG_WARN("Hook: transaction commit failed ({}:{})", __FILE__, __LINE__);                           \
    } while (0)
...
do {                                                              \
    ...
    if (_p_) {                                                    \
        o##name = (name##_t)_p_;                                  \
        if (!SFPlatform::Detour::Attach(reinterpret_cast<void**>(&o##name), reinterpret_cast<void*>(hk##name))) { \
            _ost_detour_transaction_ok_ = false;                  \
            LOG_WARN("Hook: Attach failed for {} at {}", #name, _p_); \
        } else {                                                  \
            LOG_INFO("Hook: attached {} at {}", #name, _p_);      \
        }                                                         \
    }                                                             \
} while (0)
...
#define UNINSTALL_HOOK(name)                                          \
    do {                                                              \
        if (o##name) {                                                \
            if (SFPlatform::Detour::Detach(reinterpret_cast<void**>(&o##name), reinterpret_cast<void*>(hk##name))) { \
                o##name = nullptr;                                    \
            } else {                                                  \
                _ost_detour_transaction_ok_ = false;                  \
            }                                                         \
