...
struct PackageInfo
{
	...
	int32 ChangeNumber;
	uint64 PICS_token;
	// Qualified: the field keeps Valve's name, which would otherwise shadow the
	// enum inside this class scope and change the meaning of the unqualified
	// name mid-class (rejected by GCC's -Wchanges-meaning).
...
// The offsets noted above are the 64-bit layout. The two members that move on
// 32-bit are the pointers: m_pChain stays at +0x08 (the leading union is 8B on
// both arches — m_Color[8] and m_ullValue keep it there), then the packed
// bitfield lands at +0x0C, m_unFlags at +0x10 and m_pPeer at +0x14, for 0x18
// total. Both figures are what the compiler actually produces for this
// declaration, and the 64-bit one matches Steam's steamclient64.
#if defined(__i386__) || defined(_M_IX86)
static_assert(sizeof(KeyValues) == 0x18, "KeyValues must be 24 bytes on 32-bit");
#else
static_assert(sizeof(KeyValues) == 0x20, "KeyValues must be 32 bytes on 64-bit");
#endif
...
// ── CPipeClient ────────────────────────────────────────────
// WARNING: every offset below was derived from the 64-bit Windows client, and
// the _padN runs encode 8-byte pointers and 64-bit alignment. On i386 they are
// wrong from m_clientPID onward, so reading m_szProcessName or m_clientPID on a
// 32-bit build yields garbage.
//
// This is currently unreachable on Linux: CPipeClient only ever comes from
// GetPipeClient, which has no Linux pattern, so Hooks_IPC never installs and no
// instance is ever produced. Before resolving GetPipeClient/IPCProcessMessage
// on i386, re-derive these offsets -- Valve's CValidator reflection emits each
// member's name beside a `lea reg,[this+OFFSET]`, which gives them exactly.
// A static_assert here does not work: DebugString() is a non-template member and
// is always compiled, so the assert would fire on every 32-bit build.
