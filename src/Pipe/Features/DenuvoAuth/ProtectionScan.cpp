...
#include "SFPlatform/include/ByteSearch.h"
#include "SFPlatform/include/PE.h"
...
namespace {
    ...
    const SFPlatform::PE::Section* FindLegacyDenuvoSection(const SFPlatform::PE::Image& image) {
    ...
    std::optional<DetectionMatch> TryLegacySectionString(
        ...
        const SFPlatform::PE::Image& image,
        const SFPlatform::PE::Section& section) {
        const auto matchRaw = SFPlatform::ByteSearch::FindInFile(
            module.nativePath,
            kLegacyDenuvoString,
    ...
    std::optional<DetectionMatch> TryOepPattern(
        ...
        const SFPlatform::PE::Section* section = image.SectionContainingRva(oep);
        if (!section) {
        ...
        const auto matchRaw = SFPlatform::ByteSearch::FindInFileRange(
            module.nativePath,
            section->rawOffset,
            section->rawSize,
            kDenuvoOepPattern,
    ...
    std::optional<DetectionMatch> TryProtectedBlobSection(
        ...
        for (const auto& section : image.Sections()) {
            ...
            const SFPlatform::PE::ByteBuffer sample = image.ReadRawBytes(section.rawOffset, sampleSize);
    ...
    std::vector<ModuleCandidate> EnumerateModules(PID_t pid) {
        ...
        size_t order = 0;
        for (auto module : SFPlatform::Process::EnumerateModules(pid)) {
            ...
            if (!executable && SFPlatform::Process::IsSystemModulePath(module.path)) continue;
    ...
    ProtectionScanReport RunProtectionScan(PID_t pid) {
        ...
        for (const auto& module : modules) {
            ...
            const SFPlatform::PE::Image image(module.nativePath);
            if (!image) {
