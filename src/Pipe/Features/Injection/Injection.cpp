...
#include "SFPlatform/include/RemoteProcess.h"
...
void Apply(const PipeContext& ctx) {
    ...
    auto commandLine = [&]() -> const std::optional<std::string>& {
        if (!cmdLineResolved) {
            cmdLine = SFPlatform::Process::GetProcessCommandLine(ctx.process.pid);
    ...
    for (const auto& dll : Config::injectDlls) {
        ...
        const auto status = SFPlatform::RemoteProcess::InjectLibrary(ctx.process.pid, path);
        if (status == SFPlatform::RemoteProcess::InjectStatus::Ok) {
            ...
            LOG_INJECT_WARN("inject failed pid={} appid={} status={} dll=\"{}\"",
                            ...
                            SFPlatform::RemoteProcess::ToString(status),
