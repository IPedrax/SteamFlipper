...
namespace {
    ...
    std::optional<AppId_t> AppIdFromGameIdString(std::string_view value) {
        const auto parsed = SFPlatform::Numbers::ParseUInt64(value);
...
ProcessEnvironment ReadSteamEnvironment(PID_t pid) {
    ...
    if (auto value = SFPlatform::Process::GetEnvironmentVariableValue(pid, L"SteamAppId")) {
        env.steamAppId = AppIdFromAppIdString(*value);
    }
    if (auto value = SFPlatform::Process::GetEnvironmentVariableValue(pid, L"SteamGameId")) {
        env.steamGameIdAppId = AppIdFromGameIdString(*value);
    }
    if (auto value = SFPlatform::Process::GetEnvironmentVariableValue(pid, L"SteamOverlayGameId")) {
